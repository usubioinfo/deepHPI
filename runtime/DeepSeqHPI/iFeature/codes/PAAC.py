...
USAGE:
	...
	lambda:           the lambda value, integer, defaule: 10
...
def PAAC(fastas, lambdaValue=28, w=0.05, **kw):
	...
	dataFile = re.sub('codes$', '', os.path.split(os.path.realpath(__file__))[0]) + r'\iFeature\data\PAAC.txt' if platform.system() == 'Windows' else re.sub('codes$', '', os.path.split(os.path.realpath(__file__))[0]) + 'data/PAAC.txt'
